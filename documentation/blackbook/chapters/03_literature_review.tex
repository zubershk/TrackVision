...
\]
The constant velocity model assumes $x_{t+1} = x_t + v_x \Delta t$. Process noise models acceleration uncertainty; measurement noise models detection jitter. Full covariance (6×6) propagation captures correlations between state variables (e.g., position-velocity coupling).
...
\begin{itemize}
    \item \textbf{Implemented in TrackVision:} ByteTrack two-stage association, full 6×6 covariance Kalman, Hungarian assignment, OSNet ReID with EMA fusion, track merging, EMA smoothing, occlusion interpolation, YOLOv8n detection, YOLO-World + CLIP open-vocabulary path.
