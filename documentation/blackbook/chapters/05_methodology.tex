...
For ReID (OSNet), crops are extracted from the letterboxed frame, resized to 256×128, and normalized with ImageNet statistics (mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]).
...
\begin{enumerate}
    ...
    \item \textbf{Preprocessing:} Resize 256×128, ImageNet mean/std, RGB $\to$ CHW.
...
The Kalman filter (\texttt{kalman.ts}) maintains a 6×6 covariance matrix with constant acceleration process noise.
...
\begin{itemize}
    ...
    \item Label stabilization: Majority vote over last 10 class labels
...
\begin{itemize}
    ...
    \item Scene Map: 640×480 top-down density map with live track dots.
